...
class OnboardingSessionState(TypedDict):
    """ TypedDict for the structure held in self.current_session_state """
...
class OnboardingService:
    ...
    async def start_assessment(
        self, topic: str, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if self.tech_tree_ai_instance.is_complete():
            ...
            if user_id_to_save:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to save assessment to DB: {db_err}", exc_info=True
                    )
            else:
